...
- [5A] — PR pending — Featured analysis hierarchy: first featured card spans full width with hero treatment (1.4rem title, 700 weight, 4-line summary clamp, featured-bg highlight), supporting cards below in 2-column grid with smaller treatment (0.95rem title, 500 weight, 2-line clamp). Responsive: 2-column grid at desktop, scales to 1 column at 580px. Lead card title scales 1.4rem → 1.2rem (900px) → 1.1rem (768px) → 1rem (580px). Magazine-style hierarchy signals editorial curation over chronology. CSS-only change; mobile responsive by breakpoints. — 2026-05-21
