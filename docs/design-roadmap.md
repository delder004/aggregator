...
- [3A] — PR pending — Companies as a dense static table: replace card grid with tabular view (name, category, articles, jobs, last mentioned). Pre-render alternate sort URLs (`/companies/by-articles`, `/companies/by-jobs`, `/companies/by-recent`) with sort link headers showing active sort. Table uses responsive overflow-x on desktop, hides last-mentioned column on mobile ≤768px to save space. Pagination: 40 companies per page to stay under 50KB. CSS-only changes to renderer; no database schema changes. Mobile responsive with responsive table and hidden columns at breakpoints. — 2026-05-23
