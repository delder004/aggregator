...
- [6D] — 2026-06-02 — Dark mode polish: add element-level dark mode overrides for the two hardcoded-color surfaces that don't inherit from `:root` variables. `.hero` gets `background:#060e1a` in dark mode (deeper navy, more contrast against `#09090b` body). `.newsletter-box` gets a dark-mode-adapted gradient (`#09090b → #1a0a07 → #4a1d12`) replacing the light-mode warm coral gradient. All other surfaces already use `var(--*)` variables with dark mode overrides in `:root`. CSS-only; no layout changes; mobile-clean by inheritance.
...
- [6C] — 2026-06-03 — Numbered list alt-view of The Wire: add Cards/List view toggle (`.wire-view-row`, `.wire-view-btn`) to The Wire section header on homepage and paginated pages, linking to pre-rendered `/list` and `/list/page/N` URLs. The `/list` view renders all articles as a semantic `<ol class="wire-list">` with numbered rows (`.wire-list-item`, `.wire-list-link`, `.wire-list-meta`) at 40 articles/page — compact, editorial, TOC-style. Uses `<ol start="N">` attribute for correct numbering across pages. `wireListRow()` exported from `html.ts`; `generateWireListPages()` added to `pages.ts`. No client JS; no changes to `index.ts` (KV lookup serves the new paths automatically). Mobile-clean: `<ol>` uses `padding-left:2.4rem` (collapses to `1.8rem` at ≤580px), `.wire-list-body` has `flex-wrap:wrap` and `overflow-wrap:anywhere` on titles so narrow viewports wrap cleanly.
- [8A] — 2026-06-06 — Company detail page header: replace inline-style company header block with CSS classes (`.company-detail-header`, `.company-detail-tagline`, `.company-stat-band`). h1 now uses serif stack at 2rem/800/−0.02em, responsive to 1.6rem at ≤900px and 1.3rem at ≤640px — matching the article detail page typographic scale. `.company-detail-tagline` carries the sub-title copy. `.company-stat-band` carries the category tag + website + article count + open roles + size with flex/wrap rhythm and 500-weight for heavier anchoring. CSS-only changes to `src/renderer/html.ts`; template changes in `src/renderer/pages.ts`. Mobile-clean: h1 scales down via media queries, `.company-stat-band` has `flex-wrap:wrap`, `.company-detail-tagline` has `overflow-wrap:anywhere`.
