...
- `git fetch origin <branch> && git checkout <branch>` (the branch is named in the kickoff, e.g. `agent/janitor-fix-foo`).
- Make the changes called for in the reviewer's comment.
- `npm run typecheck && npm test` to validate.
- `git add <files>`, commit with a message that names the review concern (e.g. "Address review: escape period in regex"), `git push origin <branch>`.
